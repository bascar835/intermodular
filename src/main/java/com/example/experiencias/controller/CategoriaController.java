...
public class CategoriaController {
    ...
    public CategoriaController(DataSource ds) { this.ds = ds; }

    // GET /api/categorias — solo activas (para el frontend público)
    @GetMapping
    public List<CategoriaResumen> index() {
        try (Connection con = ds.getConnection()) {
            return new CategoriaRepository(con).findResumenActivas();
    ...
    public ResponseEntity<?> experienciasPorCategoria(@RequestBody Map<String, Object> body) {
        if (body == null || !body.containsKey("categoriaId")) {
            return ResponseEntity.badRequest().body(Map.of("ok", false, "error", "categoriaId es obligatorio"));
        }
        ...
        try (Connection con = ds.getConnection()) {
            ...
            return ResponseEntity.ok(Map.of("ok", true, "categoriaId", categoriaId,
                "total", experiencias.size(), "data", experiencias));
        } catch (SQLException e) {
            throw new DataAccessException(e);
        }
    }
