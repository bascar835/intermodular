...
public class ExperienciaRepository extends BaseRepository<Experiencia> {
	...
	public String[] getColumnNames() {
		return new String[] { "id", "titulo", "descripcion", "precio", "ubicacion", "duracion_horas", "categoria_id",
				"recomendamos", "incluye" };
	}
	...
	public Object[] getInsertValues(Experiencia e) {
		return new Object[] { e.getTitulo(), e.getDescripcion(), e.getPrecio(), e.getUbicacion(), e.getDuracion_horas(),
				e.getCategoria_id(), e.getRecomendamos(), e.getIncluye() };
	}

	@Override
	public Object[] getUpdateValues(Experiencia e) {
		return new Object[] { e.getTitulo(), e.getDescripcion(), e.getPrecio(), e.getUbicacion(), e.getDuracion_horas(),
				e.getCategoria_id(), e.getRecomendamos(), e.getIncluye(), e.getId() };
	}

	// Necesario para ExperienciaRepository.findOrThrow() (usado en
	// ExperienciaImagenController)
	...
	public List<ExperienciaResumen> findResumen() {
		String sql = """
				    ...
				    ORDER BY e.titulo
				""";
		return DB.queryMany(con, sql, rs -> new ExperienciaResumen(rs.getInt("id"), rs.getString("titulo"),
				rs.getString("descripcion"), rs.getDouble("precio"), rs.getString("ubicacion"),
				(int) rs.getDouble("duracion_horas"), rs.getInt("categoria_id"),
				rs.getTimestamp("fecha_creacion") != null ? rs.getTimestamp("fecha_creacion").toLocalDateTime() : null,
				rs.getString("imagen"), rs.getString("categoria_nombre")));
	}
	...
	public List<ExperienciaResumen> findPorCategoria(Long categoriaId) {
		...
		return DB.queryMany(con, sql, rs -> new ExperienciaResumen(rs.getInt("id"), rs.getString("titulo"),
				...
				(int) rs.getDouble("duracion_horas"), rs.getInt("categoria_id"),
				rs.getTimestamp("fecha_creacion") != null ? rs.getTimestamp("fecha_creacion").toLocalDateTime() : null,
				rs.getString("imagen"), rs.getString("categoria_nombre")), categoriaId);
	}
	...
	public List<ExperienciaResumen> buscarPorTexto(String termino) {
		...
		return DB.queryMany(con, sql,
				rs -> new ExperienciaResumen(rs.getInt("id"), rs.getString("titulo"), rs.getString("descripcion"),
						rs.getDouble("precio"), rs.getString("ubicacion"), (int) rs.getDouble("duracion_horas"),
	...
	public ExperienciaDetalle findDetalle(int id) {
		String sql = """
				    SELECT e.id, e.titulo, e.descripcion, e.precio, e.ubicacion,
				           e.duracion_horas, e.categoria_id, e.recomendamos, e.incluye,
				           c.nombre AS categoria_nombre
				    FROM experiencias e
				    JOIN categorias c ON c.id = e.categoria_id
				    WHERE e.id = ?
		...
		ExperienciaDetalle base = DB.queryOne(con, sql,
				rs -> new ExperienciaDetalle(rs.getInt("id"), rs.getString("titulo"), rs.getString("descripcion"),
						rs.getDouble("precio"), rs.getString("ubicacion"), (int) rs.getDouble("duracion_horas"),
						rs.getInt("categoria_id"), rs.getString("categoria_nombre"), null, // imagenes se cargan aparte
						rs.getString("recomendamos"), rs.getString("incluye")),
				id);
		...
		List<ImagenResponse> imagenes = new ExperienciaImagenRepository(con).findByExperienciaId(id);

		// Devolver el DTO completo con imágenes
		return new ExperienciaDetalle(base.id(), base.titulo(), base.descripcion(), base.precio(), base.ubicacion(),
				base.duracion_horas(), base.categoria_id(), base.categoria_nombre(), imagenes, base.recomendamos(),
				base.incluye());
	}
}